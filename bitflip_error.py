...
def error_wire(circuit_output):
    ...
    tampered_wire = -1
    for i in range(1,len(circuit_output)):
        if circuit_output[i]>0:
            tampered_wire = i
            break

    output = [0.0 for i in range(4)]
    output[4-tampered_wire] = circuit_output[i] + circuit_output[i+4]
    output[0] = 1-output[4-tampered_wire]
    return output
